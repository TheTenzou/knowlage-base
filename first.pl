...
%isPentNumber(PNum):-PNum<1,false.

check(P1,P2):-R is P2-P1,S is P1+P2, isPentNumber(R), isPentNumber(S), write(P1), write(' '), writeln(P2).
check(_,_):-true.

run:-run(1,2).
run(I,J):-J>1000000,Inext is I+1,Js is Inext+1, run(Inext,Js),writeln('Cicle').
run(I,_):-I>1000000,fail.
run(I,J):-pentNumber(I,P1), pentNumber(J,P2), check(P1,P2), Jnext is J+1, run(I,Jnext).
